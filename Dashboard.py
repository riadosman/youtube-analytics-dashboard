...
st.caption("Analyze your channel performance and compare videos over time.")
...
def load_Data():
    ...
    df_agg.columns = (
    df_agg.columns
        .str.replace("\xad", "", regex=False)  # إزالة Soft Hyphen
...
median_agg = (
    df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo]
...
views_days = pd.pivot_table(df_time_diff_yr,index='days_published',values='Views',aggfunc=[np.mean,np.median,lambda x: np.percentile(x,80),lambda x : np.percentile(x,20)])
views_days = views_days.reset_index()
views_days.columns = [
    'days_published',
    'mean_views',
    'median_views',
...
if add_sidebar == 'Individual Video analysis':
    ...
    fig2.update_layout(title='View Cp,[arospm forst 30 days',
